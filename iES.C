...
void interrupt_synch()
{
    uart_write("\n[ISR]: SyncEx\n");
    asm volatile ("mrs x0, elr_el3; add x0, x0, #4; msr elr_el3, x0; sub x0, x0, x0;");
    return;
}
...
void uart_setup(VALUE baud)
{
    ...
    for(VALUE i = 0; i < MAX_SIZE; ++i) { uart_write_byte('\0'); }
}
...
void uart_eval(VALUE value)
{
    ...
    {
        a /= 10;
        uart_write_byte(map[(VALUE)(value / a) % 10]);
    }
}
...
VALUE memory_free(VALUE handle)
{
    ADDRESS i = 1;
    ADDRESS a = 0;
    Platform.Mailbox[i++] = 0;          //Mailbox Request
...
VALUE memory_lock(VALUE handle)
{
    ADDRESS i = 1;
    ADDRESS a = 0;
    Platform.Mailbox[i++] = 0;          //Mailbox Request
...
VALUE memory_unlock(VALUE handle)
{
    ADDRESS i = 1;
    ADDRESS a = 0;
    Platform.Mailbox[i++] = 0;          //Mailbox Request
...
VALUE qpu_setup(VALUE enable)
{
    ...
    Platform.Mailbox[i++] = 4;          //Buffer Length
    Platform.Mailbox[i++] = 4;          //Data Length
a=i;Platform.Mailbox[i++] = enable;     //Value
...
{
/*
   unsigned long long a[1024] = {0};	//Vector
   unsigned long long b = 32;		//Scalar
   unsigned long long c[1024] = {0};	//Vector
   for (int i = 0; i < 1024; ++i) { a[i] = i; }

   asm volatile
   (
       "v16ld HX(0++,0) (%[a]+=%[b]) REP64\n"
       "vmull.ss HX(0++,0), HX(0++,0), 2 REP64\n"
       "v16st HX(0++,0), (%[a]+=%[b]) REP64"
       : [c] "=rm"(c)
       : [a] "rm"(a), [b] "rm"(b)
   );
